...
class DatasetCatalog:
    ...
    def register(self, manifest_path, raw_path, adjusted_path):
        ...
        validate(raw_frame, manifest["start"], manifest["end"])
        validate(adjusted_frame, manifest["start"], manifest["end"])
        if "adj_close" not in raw_frame:
            raise ValueError("Raw snapshot has no adjusted close")
        try:
            pd.testing.assert_frame_equal(
                adjust(raw_frame), adjusted_frame, check_exact=False, rtol=1e-10, atol=1e-10
            )
    ...
    def load(self, snapshot_id):
        ...
        validate(frame, str(snapshot.start), str(snapshot.end))
